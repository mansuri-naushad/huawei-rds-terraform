...
resource "huaweicloud_networking_secgroup" "rdssecgroup" {
  name        = "rds_security_group"
  description = "security group for RDS"
}

# allow MySQL Port
resource "huaweicloud_networking_secgroup_rule" "rds_allow_mysql" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 3306
  ...
  security_group_id = huaweicloud_networking_secgroup.rdssecgroup.id
}
